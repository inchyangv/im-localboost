...
export function AdminTopBar() {
  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-ink-900/95 text-white backdrop-blur-xl">
      <div className="mx-auto flex h-16 w-full max-w-[1120px] items-center gap-3 px-4 sm:gap-5 sm:px-5">
        ...
        <div className="hidden lg:flex [&_a]:text-white/60 [&_a:hover]:bg-white/10 [&_a:hover]:text-white [&_a[aria-current=page]]:bg-white/[0.12] [&_a[aria-current=page]]:text-white">
          <NavTabs items={ADMIN_NAV} />
        </div>
        <div className="ml-auto flex items-center gap-2 sm:gap-3">
          <span className="hidden text-[12px] text-white/50 sm:inline lg:hidden xl:inline">{chainLabel(chainId)}</span>
          <EngineDot className="hidden bg-white/10 text-white/70 sm:inline-flex lg:hidden xl:inline-flex" />
          <PersonaSwitcher />
        </div>
      </div>
      <div className="scroll-none overflow-x-auto border-t border-white/10 px-3 py-2 lg:hidden [&_a]:text-white/60 [&_a[aria-current=page]]:bg-white/[0.12] [&_a[aria-current=page]]:text-white">
        <NavTabs items={ADMIN_NAV} />
      </div>
    </header>
  );
