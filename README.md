...
웹 상단의 페르소나 전환기로 계정을 고릅니다. 각 단계의 프로덕션 스크린샷은 `docs/demo/prod-step1.png` ~ `prod-step5.png`입니다.

| 단계 | 페르소나 | 화면 | 하는 일 | 기대 결과 | 스크린샷 |
| --- | --- | --- | --- | --- | --- |
| 1 | 대구시 | `/city` | 북성로에 예산 예치(500,000원) → "율 게시" (데모 전용: 북성로 10% 고정 체크) | 북성로 카드와 `/` 약도에 10.0% | [step1](docs/demo/prod-step1.png) |
...
- **고정 율 override**: `/city`의 "데모 전용: 북성로 10% 고정" 체크는 `/rates/publish`에 `overrides: {"4": 1000}`을 보냅니다. 엔진이 계산한 다른 상권 율은 그대로 게시됩니다.
