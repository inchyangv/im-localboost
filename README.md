...
소비자는 상단의 "지갑 연결"로 본인 지갑(MetaMask, Kaia Wallet)을 씁니다. 처음 연결한 지갑은 "은행 등록하고 시작 자금 받기"를 누르면 엔진의 `POST /onboard`가 은행 키로 personId를 연결하고 iMKRW 100,000원과 수수료용 코인을 보냅니다(지갑당 KST 하루 1회). 발표용 데모 계정(소비자 1~5, 가맹점 1~3, 대구시, 은행)은 상단의 데모 계정 메뉴에 남아 있으며, 고르면 그 역할의 화면으로 이동합니다. 각 단계의 프로덕션 스크린샷은 `docs/demo/prod-step1.png` ~ `prod-step5.png`이며, 2026-09-18 UI 재설계 이전 화면으로 찍은 것이라 배치와 색은 현재 배포와 다릅니다. 단계와 기대 결과는 같습니다.

| 단계 | 페르소나 | 화면 | 하는 일 | 기대 결과 | 스크린샷 |
| --- | --- | --- | --- | --- | --- |
| 0 | 내 지갑 | `/` | "Kaia Wallet 연결" → 지갑에서 승인 → (필요하면) Kairos로 네트워크 전환 → "은행 등록하고 시작 자금 받기" | 지갑 카드에 잔액 100,000원, 은행 미등록 배지 사라짐. 이후 2~3단계를 이 지갑으로 진행해도 결과는 같음 | - |
...
1. 상단 "지갑 연결"을 누르고 MetaMask 또는 Kaia Wallet에서 승인합니다. 앱 체인이 아니면 네트워크 전환(없으면 추가) 요청이 뜹니다. Kairos: chainId 1001, RPC `https://public-en-kairos.node.kaia.io`, 통화 KAIA.
2. "은행 등록하고 시작 자금 받기"를 누릅니다. 은행이 personId 연결, iMKRW 100,000원 발행, 수수료용 코인 0.2개 전송을 순서대로 보냅니다.
3. 가맹점과 금액을 고르고 "결제하기"를 누르면 지갑에서 승인(approve, 처음 한 번)과 결제 서명을 확인합니다. 결과 카드와 잔액은 데모 계정과 같습니다.
4. 위험 모델은 정시 직후 몇 초 안의 결제에 민감하게 반응합니다(`secs_since_rate_change`). 웹은 매시 정각부터 45초까지는 카운트다운을 보여 주며 기다렸다가 `/attest`를 호출하므로, 보통의 첫 결제는 tier 0으로 즉시 적립됩니다.

로컬(Hardhat)에서는 지갑에 chainId 31337, RPC `http://127.0.0.1:8545`를 추가하면 같은 흐름이 됩니다. 노드를 다시 띄우면 지갑의 nonce 캐시를 초기화해야 합니다.
